...
#### Traffic Speeds Metadata Schema (13 Columns)

| Column Name | Description | API Field Name | Data Type |
| --- | --- | --- | --- |
| ID | Unique record identifier. | `id` | Text |
| SPEED | Reported traffic speed value for the link. | `speed` | Text |
| TRAVEL_TIME | Reported travel time value for the link. | `travel_time` | Text |
| STATUS | Feed/status indicator for the reported measurement. | `status` | Text |
| DATA_AS_OF | Timestamp indicating when the measurement was captured. | `data_as_of` | Floating Timestamp |
| LINK_ID | Numeric/text identifier for the traffic link segment. | `link_id` | Text |
| LINK_POINTS | Raw geometry point string for the traffic link. | `link_points` | Text |
| ENCODED_POLY_LINE | Encoded polyline representation of link geometry. | `encoded_poly_line` | Text |
| ENCODED_POLY_LINE_LVLS | Encoded polyline level metadata. | `encoded_poly_line_lvls` | Text |
| OWNER | Source/owner designation associated with the link record. | `owner` | Text |
| TRANSCOM_ID | TRANSCOM identifier associated with the link. | `transcom_id` | Text |
| BOROUGH | NYC borough associated with the link record. | `borough` | Text |
| LINK_NAME | Human-readable road/link name. | `link_name` | Text |
