...
## 1) Code Setup
- [ ] Split timing scopes in `BenchmarkHarness` so validation work is not included in `query_ms`.
- [ ] Add explicit benchmark mode toggle: correctness-validation mode vs performance-only mode.
- [ ] Add `validation_ms` and `validation_ingest_ms` columns to benchmark CSV output.
...
- [ ] Re-check optimized query scaling (1/2/4/8 threads) after timing-scope fix.
